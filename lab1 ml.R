...
student_data$is_pass <- as.factor(ifelse(student_data$final_result == "Pass", 1, 0))
...
logit_model <- glm(is_pass ~ disability_status + imd_numeric, family = binomial(link = "logit"), data = train_set)
...
true_outcomes <- as.numeric(test_set$is_pass) - 1  # Adjusting factor levels from 1 to 0 and 1 for comparison
